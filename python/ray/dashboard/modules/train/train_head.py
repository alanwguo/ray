...
class TrainHead(dashboard_utils.DashboardHeadModule):
    ...
    async def get_train_runs(self, req: Request) -> Response:
        ...
        if stats_actor is None:
            ...
            try:
                ...
                train_runs_with_details = await self._add_actor_status_and_update_run_status(train_runs)
    ...
    async def _add_actor_status_and_update_run_status(self, train_runs):
        from ray.train._internal.state.schema import ActorStatusEnum, RunStatusEnum, TrainRunInfoWithDetails, TrainWorkerInfoWithDetails
        ...
        for train_run in train_runs.values():
            ...
            for worker_info in train_run.workers:
                ...
                if actor:
                    gpus = [gpu for gpu in actor["gpus"] if worker_info.pid in [process["pid"] for process in gpu["processes"]]]
                    ...
                    worker_info_with_details = TrainWorkerInfoWithDetails(**worker_info.dict())
            ...
            logger.info(f"Train run {train_run_with_details.name} has {len(worker_infos_with_details)} workers.")
            ...
            controller_actor_status = actors.get(worker_info.actor_id, None).get("state")
            if (
