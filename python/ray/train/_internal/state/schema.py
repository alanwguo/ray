...
class TrainWorkerInfoWithDetails(TrainWorkerInfo):
    """Metadata of a Ray Train worker."""
    process_stats: Optional[ProcessStats] = Field(..., description="Process stats of the worker.")
    gpus: List[GPUStats] = Field(default_factory=list, description="GPU stats of the worker. Only returns GPUs that are attached to the worker process.")
